...
**它是什么：** UVP 是 0–100 的本地相对日照暴露潜势。数值越高，意味着这一天太阳高度更高、有效日照时数更长，因此直接日照暴露潜势更大。
...
flowchart TD
    ...
    E -- 否 --> G["保留原 S"]
    F --> H["UVP = 100 × S × sin(h) / 14.131644"]
    G --> H
    H --> I["按历史 20/40/60/80 分位数分级"]
    I --> J["输出：UVP 数值、低至高暴露潜势等级"]
```
...
UVP = 100 × S × sin(h) / 14.131644170491846
```

当前历史阈值为 27.759、35.833、49.046、66.786，对应低、较低、中等、较高、高暴露潜势。它们仅表示“相对于茶卡历史条件”的位置，不是国际健康风险分级。
